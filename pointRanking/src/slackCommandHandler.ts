...
async function handleMonthRanking(
  ...
  try {
    ...
    // ランキングデータを整形
    const rankingBlocks = rankingData.map((entry, index) => {
...
async function handleTotalPoints(
  command: SlackCommandMiddlewareArgs["command"]
) {
  try {
    ...
    // ポイントデータを整形
    const pointsBlocks = pointsData.map((entry, index) => {
      const medal =
        index === 0
          ? ":金メダル:"
          : index === 1
...
async function handleMyPoints(command: SlackCommandMiddlewareArgs["command"]) {
  try {
    ...
    // ユーザーの順位を計算
    const userRank =
      allUsersData.findIndex((user) => user.user_id === userId) + 1;
