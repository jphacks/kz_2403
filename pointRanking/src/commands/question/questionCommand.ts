import { BlockAction, SlackActionMiddlewareArgs, SlackCommandMiddlewareArgs, SlackViewMiddlewareArgs, ViewSubmitAction } from "@slack/bolt";
import { SlackCommandProps } from "../command";
import { WebClient } from "@slack/web-api";

export default function questionCommand({slackBot, slackClient}: SlackCommandProps) {
  slackBot.command("/question", async({ command, ack }: SlackCommandMiddlewareArgs) => {
    try {
      await ack();
  ...
  slackBot.view("question_modal", async ({ ack, view, client }: SlackViewMiddlewareArgs<ViewSubmitAction> & { client: WebClient }) => {
    try {
      await ack();
  ...
  slackBot.view("reply_modal", async ({ ack, body, view, client }: SlackViewMiddlewareArgs<ViewSubmitAction> & { client: WebClient }) => {
    try {
      await ack();
  ...
  slackBot.action("reply_button", async ({ ack, body, client }: SlackActionMiddlewareArgs<BlockAction> & { client: WebClient }) => {
    try {
      await ack();
